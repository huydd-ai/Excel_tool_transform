...
def _step(action, target="", params="", suite="S", step_no=1, row=2):
    return Step(suite_id=suite, step_no=step_no, action=action,
                excel_row=row, target=target, params=params, expected="")
...
def test_swipe_invalid_json_returns_todo():
    gen = AirtestGenerator()
    lines, issue = gen._HANDLERS["SWIPE"](gen, _step("SWIPE", params="not json"), _ctx())
    assert issue is not None
...
def test_scroll_up_generates_two_lines():
    gen = AirtestGenerator()
    lines, issue = gen._HANDLERS["SCROLL"](gen, _step("SCROLL", params='{"direction":"up"}'), _ctx())
    assert issue is None
    assert len(lines) == 2
...
def test_scroll_invalid_direction_returns_todo():
    gen = AirtestGenerator()
    lines, issue = gen._HANDLERS["SCROLL"](gen, _step("SCROLL", params="diagonal"), _ctx())
    assert issue is not None
...
def test_long_press_generates_long_click():
    gen = AirtestGenerator()
    lines, issue = gen._HANDLERS["LONG_PRESS"](gen, _step("LONG_PRESS", target="btn"), _ctx())
    assert issue is None
...
def test_sleep_json_seconds():
    gen = AirtestGenerator()
    lines, issue = gen._HANDLERS["SLEEP"](gen, _step("SLEEP", params='{"seconds": 3}'), _ctx())
    assert issue is None
...
def test_snapshot_no_params_uses_suite_id():
    gen = AirtestGenerator()
    step = Step(suite_id="TC_MISSION_1", step_no=1, action="SNAPSHOT",
                excel_row=2, target="", params="", expected="")
...
def test_stop_app_generates_stop_app_call():
    gen = AirtestGenerator()
    lines, issue = gen._HANDLERS["STOP_APP"](gen, _step("STOP_APP"), _ctx(pkg="com.test"))
    assert issue is None
